...
Computes LOO attribution over 50 episodes.
For sequential tasks, only LOO is meaningful while perturbation and Shapley
are identical to LOO because we remove the entire agent contribution.
"""
...
def leave_one_out_overcooked(agent_a, agent_b, env_wrapper):
    """Compute LOO attribution by re‑running episodes with one agent ablated."""
    original_reward = run_episode(agent_a, agent_b, env_wrapper)
    # Ablate agent A (replace with STAY)
...
def main():
    ...
    for ep in range(episodes):
        ...
        attr_a, attr_b = leave_one_out_overcooked(agent_a, agent_b, env)
    ...
    print(f"Leave‑One‑Out     {np.mean(all_attr_a):.4f}    {np.mean(all_attr_b):.4f}")
    print("Note: Perturbation and Shapley are identical to LOO for sequential tasks so only LOO is reported.")
